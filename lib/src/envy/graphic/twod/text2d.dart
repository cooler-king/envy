...
class Text2d extends Graphic2dNode {
  ...
  void renderIndex(int index, CanvasRenderingContext2D ctx, {HitTest? hitTest}) {
    ...
    ctx.textAlign = _align.value;

    final _baseline = textBaseline.valueAt(index);
    ctx.textBaseline = _baseline.value;
    ...
    if (hitTest != null) {
      ...
      ctx.closePath();
      if (_fill && ctx.isPointInPath(hitTest.x, hitTest.y) || _stroke && ctx.isPointInStroke(hitTest.x, hitTest.y)) {
        hitTest.hit = true;
      }
      return;
    }
