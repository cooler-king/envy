...
abstract class HtmlNode extends GroupNode with DynamicNode {
  ...
  final LinkedHashMap<DomNodeCoupling, Node> domNodesMap = new LinkedHashMap<DomNodeCoupling, Node>();
  ...
  final LinkedHashMap<DomNodeCoupling, Node> prevDomNodesMap = new LinkedHashMap<DomNodeCoupling, Node>();
  ...
  void set parent(EnvyNode node) {
    ...
    if (node != super.parent && super.parent != null) domNodesMap.values.forEach((Node n) {
      n.remove();
    });
  ...
  void updateDom() {
    ...
    if (newDomSize != _prevDomSize || _populationStrategyChanged) {
      ...
      int parentDomCount = parentHtml?.domNodesMap?.length ?? 0;
  ...
  void _manageDomNodes(int parentDomNodeCount, int childCount) {
    ...
    List<DomNodeCoupling> remainingCouplings = new List.from(domNodesMap.keys);
